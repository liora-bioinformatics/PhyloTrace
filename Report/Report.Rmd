...
file_ext <- ifelse(report_vars[["plot"]][1] == "MST", ".jpg", ".svg")
knitr::include_graphics(paste0(report_vars[["plot"]][1], file_ext))
```
\
...
if (report_vars[["report_df.Include"]][1]) {
  kable(
    select(report_vars[["entry_table"]], report_vars[["table_columns"]]),
    row.names = FALSE,
    linesep = ""
  ) %>%
    kable_styling(
    ...
    row_spec(1:nrow(report_vars[["entry_table"]]), extra_css = "padding: 3px")
}
```
...
if (!is.null(report_vars[["na_handling"]])) {
  df <- data.frame(
    Item = c(
      if (report_vars[["report_df.Include"]][9]) {
        "Tree construction"
      },
      if (report_vars[["report_df.Include"]][10]) {
        "Distance algorithm"
      },
      if (report_vars[["report_df.Include"]][11]) {
        "Missing value handling"
      },
      if (report_vars[["report_df.Include"]][12]) {
        "PhyloTrace version"
      },
      if (report_vars[["report_df.Include"]][12]) {
        "pblat version"
    ...
    Content = c(
      ...
      if (report_vars[["report_df.Include"]][10]) {
        report_vars[["distance"]]
      },
      if (report_vars[["report_df.Include"]][11]) {
        report_vars[["na_handling"]]
      },
      if (report_vars[["report_df.Include"]][12]) {
        report_vars[["version"]][1]
      },
      if (report_vars[["report_df.Include"]][12]) {
        report_vars[["version"]][2]
      }
    )
  )
} else {
...
if (report_vars[["report_df.Include"]][8] & show_analysis) {
  kbl(report_vars[["scheme"]]) %>%
    kable_styling(position = "right", font_size = 10) %>%
    kable_paper(full_width = F) %>%
    column_spec(1, bold = T, border_right = T) %>%
