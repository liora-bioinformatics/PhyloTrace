...
Each mode also switches to a base map suited to it — a plain grey canvas under a
choropleth, a light one under a heatmap — which you can override under
**Basemap > Base map**. Choropleth mode draws no base tiles at all, only place
labels, because a textured background reads through a partly transparent fill;
the Base map picker is hidden there for that reason.
...
The map's tabs change with the mode, but the full set is:
...
**Color > Variable Mapping** encodes any metadata field or custom variable: switch
on **Color by variable** and pick one under **Variable** (@sec-viz-mapping). In
Charts mode the same variable splits each location's mini-chart by category
instead of colouring a point. The choropleth shades by isolate count using
standard country polygons.
