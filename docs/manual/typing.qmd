...
The left sidebar *is* the pipeline (@fig-ui-typing-sidebar). It holds four
numbered sections in the order a genome runs through them, and each carries a
**ⓘ icon** that opens a full explanation of that step in a dialogue.
...
Both feed **cgMLST and classical MLST alike** — the classical-MLST search runs
with the same identity and coverage values as the cgMLST call
(@sec-typing-classical), so raising or lowering them moves both together. Only
the **AMR screen** has its own internal criteria and is unaffected.
:::
...
Each file goes through the same sequence of steps, in order (@fig-typing). They
are genuinely sequential — the cgMLST call does not launch the classical MLST
call, and neither launches the AMR screen; each simply follows the previous one
on the same file.
...
flowchart TB
  ...
  %% Linked cluster-to-cluster, not node-to-node: an edge aimed at a node
  %% *inside* a subgraph is drawn straight through the subgraph's title text.
  %% Targeting the cluster stops the arrow at its boundary instead.
  PREP --> RUN
  ...
  RUN --> RB["Results read back<br/>and stored"]
...
The **Results** section fills in row by row as the run proceeds
(@fig-ui-typing-results) — one row per assembly, thirteen columns. Every column
header carries a hover tooltip explaining it; the table below is the same
...
The assembly fingerprint is a *one-way* indicator. A match proves two runs
consumed assemblies with content-identical sequence — the fingerprint ignores
headers, contig order and letter case, so a renamed or reformatted copy of the
same assembly still matches; a mismatch proves nothing — re-assembling the same
reads with a different assembler produces different contigs and therefore a
different fingerprint. PhyloTrace therefore uses it to *flag* likely duplicates
for your attention, and never as a hard rule that could block a legitimate
re-analysis (@sec-concepts-hashing).
:::
