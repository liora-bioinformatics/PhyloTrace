...
The **Import** panel merges another PhyloTrace database into the one you have
open.
...
loci — and, the strongest test, the **same reference genome**.
...
You then decide, per isolate, whether to **import**, **skip** or **rename** it —
PhyloTrace suggests a free name for the rename case. A preview reports exactly
what the merge would do, in counts, **before anything is written**.

Custom variables from the incoming database are adopted only where you say so and
where the declared types are compatible with your own.
...
When the merge finishes, PhyloTrace offers to reload the database so every panel
picks up the new isolates.
:::
...
The **Export** panel writes a **new** database containing a chosen set of
isolates and a chosen set of metadata columns — for a colleague to import, or as
a trimmed working copy. A preview reports the isolate, allele and locus counts
before anything is written. Result tables such as classical MLST and AMR are
included when they exist and you select them.
...
The exported file is a complete, standalone PhyloTrace database: the recipient
...
- A merge is offered only between databases built from the same scheme, verified
  down to the reference genome.
- Collisions are previewed and resolved per isolate; the merge runs on a copy and
  leaves your original as a timestamped backup.
- Export builds a new database containing only the isolates and columns you
  select — withheld data is never written.
- Profile tables from partners can be staged for distance comparison but never
  become typed isolates.
