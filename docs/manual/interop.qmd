...
the main area fills with a pass / warn / fail grid of compatibility checks. The
remaining sidebar groups — **Metadata fields**, **Custom variables**, **Analysis
results** — decide what comes across, and **Merge into database** at the bottom
performs it. It stays disabled until the checks pass.
...
row of their own**, each with a dropdown offering **import**, **skip** or
**rename**, and a text field pre-filled with a free name — the original with
`_ext` appended — for the rename case.
...
**Database Browser > Export** is Import's mirror image: the same right-hand
sidebar shape, worked down from the top. **Export type** is again the control
