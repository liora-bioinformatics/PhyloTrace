...
tab strip — plus one tab per plot you have made (@fig-ui-viz-new). The New tab
is the creator form, and it has three parts:
...
Every plot tab has a control sidebar on each side (@fig-ui-viz-layout), and
which side a control is on tells you how it behaves:
...
row per isolate with a tick box (@fig-ui-viz-selection). It carries:
...
| **Map** | **Color > Variable Mapping**; **Charts > Variable** in Charts mode | Marker colour, or the segments of a per-location mini-chart |
...
**Export Plot > Save plot** opens the export dialogue (@fig-ui-viz-export). It
asks two questions and hides a third.
