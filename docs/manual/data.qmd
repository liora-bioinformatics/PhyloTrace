...
Loading a file is the moment the app comes to life: the four workspace panels
appear (@sec-app-panels) and every view starts reading from it. Before that
happens, PhyloTrace runs a short set of housekeeping checks on the file so that
every panel is guaranteed to be looking at the same, current picture:
...
This is not an arbitrary restriction: those names are used as filenames when you
...
Free text *is* allowed where a value is only ever displayed — plot titles,
analysis names, descriptions, and metadata values such as a collection site.
...
- **Deleting isolates is deliberate and immediate.** The Browse Entries panel
  removes them from the database, and asks whether the allele sequences they
  contributed should be kept for future comparisons (@sec-browser).
